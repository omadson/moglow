...
class Permutation(Transform):
    ...
    @staticmethod
    def _permute(inputs, permutation, dim, point=False):
        ...
        if point:
            return outputs, logabsdet, torch.zeros_like(inputs)
        return outputs, logabsdet

    def forward(self, inputs, conds=None, context=None, point=False):
        return self._permute(inputs, self._permutation, self._dim, point=point)
